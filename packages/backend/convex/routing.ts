 ...
 */
export function deriverHotes(declare: string | null | undefined): Hotes {
...
export const hotes = query({
  ...
  handler: async (ctx, args): Promise<Hotes> => {
    await assertSharedSecret(args.secret, process.env.ROUTING_SECRET)

    // La ligne entière n'est jamais rendue : on n'en lit qu'un champ, et
    // on n'en fait sortir que ce qui a passé `normaliserHote`.
    const settings = await ctx.db.query("settings").first()
    return deriverHotes(settings?.declaredDomain)
  },
})
