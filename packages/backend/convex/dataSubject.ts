...
const LIMITES_CONNUES = [
  ...
  "Le journal d'administration (`auditLog`) n'est PAS inclus, et il peut contenir quelque chose. Pour un VISITEUR, non : la suppression d'une fiche de contact n'y inscrit que l'identifiant interne de la ligne, jamais l'adresse ni le nom — c'est précisément pour ne pas défaire l'effacement. Pour un ADMINISTRATEUR, oui : un changement de rôle ou une suppression de compte y inscrit son adresse électronique, et les lignes dont il est l'AUTEUR y portent son nom d'affichage, rattachées à son identifiant Better Auth et non à son adresse. Ce n'est pas lu ici parce qu'une ligne de ce journal nomme aussi l'administrateur qui a fait le geste : la rendre telle quelle divulguerait l'identité d'un tiers en croyant répondre à un droit. Une demande d'accès émanant d'un administrateur se traite donc à la main, par `npx convex run` sur la table `auditLog`, en décidant au cas par cas ce qui relève de la personne qui demande et ce qui relève de quelqu'un d'autre.",
]
