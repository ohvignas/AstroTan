...
import { isServedByRoute } from "./servedPaths"
...
describe("les chemins qu'un fichier de route sert déjà", () => {
  ...
  test("une route dynamique possède tout ce qui est sous son préfixe", () => {
    // `/blog/[slug].astro` répond pour n'importe quel article : une
    // redirection depuis `/blog/x` serait masquée alors même qu'aucun
    // chemin exact ne correspond.
    expect(isServedByRoute("/blog/mon-article")).toBe(true)
    expect(isServedByRoute("/blog/nimporte-quoi")).toBe(true)
  })
